...
/**
 * Contributions & Payments landing page (`/admin/contributions`).
 *
 * Contributions, payments, and reminders are committee-scoped in the documented
 * API, so this page lists the committees the admin manages (GET /committees)
 * and links each to its contribution & payment management view. Covers
...
export function ContributionsAdmin() {
  ...
  return (
    <AdminPageContainer
      ...
      breadcrumbs={[{ label: 'Contributions & Payments' }]}
      actions={
    ...
    >
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3, maxWidth: 720 }}>
        Contribution and payment management is organised per committee. Choose a committee to review
        member contribution status by cycle, verify or reject payment claims, and send payment
        reminders.
      </Typography>
      ...
      {isLoading ? (
        ...
        <Paper sx={{ py: 8, px: 3, textAlign: 'center' }}>
          ...
          <Typography variant="body2" color="text.disabled" sx={{ mb: 3 }}>
            Create a committee first — you can manage its contributions and payments once it exists.
          </Typography>
        ...
        <Grid container spacing={2}>
          {committees.map((committee) => (
            <Grid key={committee.id} size={{ xs: 12, sm: 6, md: 4 }}>
              <Box
                component={Link}
                href={`/admin/contributions/${committee.id}`}
                sx={{
                  display: 'flex',
              ...
              >
                ...
                <Typography variant="body2" color="text.secondary">
                  Contribution {formatCurrency(committee.contributionAmount)} ·{' '}
