...
/**
 * Lottery landing page (/admin/lottery).
 *
 * Lists the admin's committees and links each to its lottery management
 * view where lottery history, eligible cycles, and lottery execution
 * are available.
 */
export function LotteryAdmin() {
  ...
  return (
    <AdminPageContainer
      title="Lottery"
      breadcrumbs={[{ label: 'Lottery' }]}
      actions={
        <Button component={Link} href="/admin/cycles" variant="outlined">
          Go to Cycles
        </Button>
      }
    >
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3, maxWidth: 720 }}>
        Lottery management is organised per committee. Choose a committee to view lottery
        history, check cycle eligibility, and run the lottery draw for eligible cycles.
      </Typography>
      ...
      {isLoading ? (
        ...
        <Paper sx={{ py: 8, px: 3, textAlign: 'center' }}>
          ...
          <Typography variant="body2" color="text.disabled" sx={{ mb: 3 }}>
            Create a committee first — you can manage its lottery once it has cycles.
          </Typography>
        ...
        <Grid container spacing={2}>
          {committees.map((committee) => (
            <Grid key={committee.id} size={{ xs: 12, sm: 6, md: 4 }}>
              <Box
                component={Link}
                href={`/admin/lottery/${committee.id}`}
                sx={{
                  display: 'flex',
