...
export function AuditLogsAdmin() {
  ...
  return (
    <AdminPageContainer title="Audit Logs" breadcrumbs={[{ label: 'Audit Logs' }]}>
      <Typography color="text.secondary" sx={{ mb: 3 }}>Choose a committee to view its read-only audit history and chronological timeline.</Typography>
            ...
            {data?.data.map((committee) => (
              <Grid key={committee.id} size={{ xs: 12, sm: 6, md: 4 }}>
                <Paper sx={{ p: 3, height: '100%' }}>
                  ...
                  <Button component={Link} href={`/admin/audit-logs/${committee.id}`} variant="outlined">View audit history</Button>
                </Paper>
              </Grid>
            ))}
          </Grid>
